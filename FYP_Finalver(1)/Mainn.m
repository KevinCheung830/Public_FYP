...
spectralEffSVD= zeros(implementation,numel(snr));
SVD_SqErr = zeros(implementation,numel(snr));
spectralEff_BS = zeros(implementation,numel(snr));
...
%       W_opt = W_opt(1:N_s,:);
        ...
        SVDEff = log2(real(det( I + (rho/N_s)*Temp_SVD*inv(R_SVD))));
        ...
        grid_Save = Ar'*H*At;
        for iiii = 1:Nrf
            %Retrieve the index for the max gain in the Grid_
            [value_,index_] = max(abs(grid_),[],'all');
            [row_,col_] = ind2sub(size(grid_),index_);

           %Append the max gain according to index
            W_BS(iiii,:) = Ar(:,row_)';
            F_BS(:,iiii) = At(:,col_);
                
            
            % disp('BeamSteeringCurrentPeak');
            % disp('Row:');
            % disp(col_);
            % disp('Column');
            % disp(row_);
            % figure
            % surf(abs(grid_));
            % title('BrutalforceSerach of Beamsteering')
            % xlabel('Nt')
            % ylabel('Nr')
            % zlabel('Gain')
...
figure
hold on
l1 = plot(snr,spectralEffSVD,'-o');; %SVD with blue dotted x
l2 = plot(snr,spectralEff_OMP,'-*');    %Hybrid Precoding with  orange line
l3 = plot(snr,spectralEff_BS,'c'); 
